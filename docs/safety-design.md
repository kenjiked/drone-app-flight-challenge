...
|---|---|---|---|
| **巡回範囲の上限＝安全に戻れる目安(A)** | ホーム→最遠点が「安全に戻れる目安」内か | **導出値**: `min(目視200m, 電池の予備で帰れる距離)×0.9`＝既定180m（下記「上限の根拠」） | ✅ 実装（`geo_safety.safe_return_limit_m`／level=block） |
...
### 範囲上限の根拠（2026-07-04 改訂: 固定135m → 導出値）
「135m は何の根拠？」への答えを持てるよう、固定値をやめて**理由のある値からの導出**に変更した
（`geo_safety.safe_return_limit_m()`）:

```
安全に戻れる目安 = min( 目視の目安 200m,
                        電池の予備だけで最遠点から帰れる距離 ) × 0.9
                 = min( 200m, 720s×40%×5m/s = 1440m ) × 0.9 = 180m（既定）
```

- **目視 200m**: 操縦位置(離発着地点)から機体を見失わない距離の目安（VLOS）。
- **電池の予備**: 電池チェックが飛行全体を「電池の60%以内」に制限して**必ず40%残す**。
  その予備だけで最遠点からでも帰還できることを保証する（= endurance×0.4×巡航速度）。
- **×0.9**: 硬い上限の一歩手前で余裕を持つ係数（自作Lua監視が標準フェンスの0.9で
  優しくRTLする設計と同じ思想）。
- 旧135m は「デモ用 parm の FENCE_RADIUS=150（既定ルート113m+余裕の丸め） × Lua係数0.9」
  由来の**設計初期値**で、法令根拠ではなかった（航空法の150mは高さ制限で水平距離ではない）。
- 実飛行時のジオフェンスは従来どおり**ルートに自動フィット**（最遠点+30m）で別途投入される。
  本値は「この製品として許すみまわり範囲の上限キャップ」。
...
## 9. ダイバート（避難地点）設計（D25 / 2026-07-05）

> 航空の「代替飛行場（オルタネート）」の発想: 帰還先は常にホームとは限らない。
> ホームまで電池が持たない・ホーム側に危険がある場合、**より近く安全な避難地点へ転換**する。

### 先行事例調査（実ソース確認済み）

- 本家に **`AP_Rally`（ラリーポイント）** が存在。Copter の RTL は既に
  `mode_rtl.cpp:471` で `calc_best_rally_or_home_location()` を呼び、
  **最寄りのラリーポイント or ホーム**を帰還先に選んでいる。
- → 避難地点を機体に登録しさえすれば、中止・フェンス突破・電池FSなど
  **全てのRTLが自動的にダイバート対応になる**（コード0行）。
- Lua からも `rally:get_rally_location(index)` / `vehicle:set_target_location()` が使える
  （`AP_Scripting/docs/docs.lua` で確認）。

### 実装ロードマップ（例によって段階式）

| Step | 内容 | 差別化/学び |
|---|---|---|
| **1. アプリ統合** | UI②に「🅿 避難場所」を地図タップで登録（複数可）→ MAVLink（MISSION_TYPE_RALLY）で機体へ登録。precheck に「避難地点の安全確認」（禁止ゾーン・鉄道・送電線近接チェックを避難地点にも適用）を追加 | 機能は本家、**非エンジニアが使える化**がうちの主戦場（D11） |
| **2. Lua** | **エネルギー認識ダイバート**: 毎秒「残電池×巡航速度で届く距離」を計算し、ホームに届かない時だけ届く避難地点へ `set_target_location`。本家の"最寄り優先"を超える付加価値 | 本家に無い判断ロジックの自作 |
| **3. C++（D24 Lv2連携）** | Step2の判断を本体へ移植 — `AP_Rally::calc_best_rally_or_home_location` を読み、電池考慮版に拡張 or みまわりウォッチャーに統合 | フライトコード深掘りの実弾教材 |

### 注意点（設計時に詰める）

- 避難地点の**高度と着地の安全**: ラリーポイントは「その上空へ行く」。自動着陸まで
  させるか（RALLY_FLAGSの挙動確認）、ホバリングで人の判断を待つか。
- precheck との整合: 避難地点が「安全に戻れる目安」の中にあるか、避難地点自体が
  禁止ゾーン/鉄道近くでないかを①のチェックに載せる。
- UIの言葉: 非エンジニア向けには「ラリーポイント」ではなく **「避難場所」**。
