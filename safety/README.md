...
未アーム中に電池不足/位置未確定なら `PS: batt 20%<40%` / `no position (GPS/EKF)` の理由付きで**離陸がブロック**される。
飛行中(AUTO/GUIDED)に姿勢/電池/範囲が閾値を超えると `PatrolSafety: out of range 60m>54m -> RTL` のように**自動で RTL** に切り替わる。

> GCS メッセージは短い ASCII に統一している。MAVLink STATUSTEXT が 50 バイト上限で、日本語(UTF-8)は
> 途中切れ・文字化けするため。非エンジニア向けの日本語表示は Web UI 側の役割（機体側は技術メッセージ）。
